...
using InMemoryWorkerBalancer.Remote.Clients.NamedPipe.Options;
using InMemoryWorkerBalancer.Remote.Protos;

namespace InMemoryWorkerBalancer.Remote.Clients.NamedPipe;
...
public sealed class NamedPipeRemoteWorkerClient : IAsyncDisposable
{
    ...
    {
        _options = options;
        _channel = new NamedPipeChannel(options.ServerName, options.PipeName);
        _client = new WorkerHub.WorkerHubClient(_channel);
    }
